...
#处理数据
def process_data():
    d = np.loadtxt("./ML_Module/test_data/base.tmp",delimiter = ',')
    s = 0 #开始行
    e = len(d) #结束行
    ...
    # 保存到历史缓存文件
    col = len(d[0])
    f =  open("./ML_Module/test_data/history.tmp", 'a+')
    for i in range(s,e):
    ...
    np.savetxt("./ML_Module/test_data/test_tmp.csv",tmp,fmt='%.2f',delimiter=',')

#极大值归一化
def normalize_data():
    data = np.loadtxt("./ML_Module/test_data/test_tmp.csv", delimiter=',')
    max_x = np.loadtxt("./ML_Module/train_data/max_x.csv", delimiter=',')
    min_x = np.loadtxt("./ML_Module/train_data/min_x.csv", delimiter=',')
    num = [0]*len(max_x)
    ...
    for i in range(len(data)):
        if num[i] != 0:
            ...
            data[i] = 1.0
    # 保存测试向量    
    np.savetxt("./ML_Module/test_data/test_x.csv", data,fmt='%.3f',  delimiter=',')
...
class BP_NeuralNetwork_Test:
    ...
    def test(self):
        #加载保存的模型
        self.inputs = np.loadtxt("./ML_Module/test_data/test_x.csv", delimiter=',')
        ...
        if(self.layers[self.layer_num][0]>0.5):
            ...
            print("[安全]无电力攻击风险 ")
        #加载关键数据
        d = np.loadtxt("./ML_Module/test_data/test_tmp.csv")
        net = d[12]
        if(net < d[13]):
            net = d[13]
        net /= 1024
        ...
        print(d[0],d[7],d[8],d[10],round(net,3)) 
