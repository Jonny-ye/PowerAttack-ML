...
def process_data(group_n):
    ...
    for i in range(t):
        ...
        #生成一个样本数据
        tmp.append([s_watts_avg,s_watts_max,s_watts_ms,load15,load5,load1,t_ms,c_avg,c_max,c_ms,m_avg,m_max,m_ms,ni_ms,no_ms,disk_max])
...
def normalize_data():
    ...
    for i in range(rows):
        for j in range(cols):
            if num[j] != 0.0:
                data[i][j] = (data[i][j] - min_x[j])/num[j]
            else:
                data[i][j] = 1.0
    # 保存最大值数组和归一化样本数据
    np.savetxt("./train_data/max_x.csv",max_x,fmt='%.4f',delimiter=',')
    np.savetxt("./train_data/min_x.csv",min_x,fmt='%.4f',delimiter=',')
